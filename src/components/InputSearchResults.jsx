...
function InputSearchResults({
  ...
  return (
    <ul
      className={`${isPending || isError || results?.length < 3 ? "scrollNone" : "scroll"} min-h-30 absolute top-20 z-30 max-h-80 w-full overflow-y-auto rounded-lg border-solid bg-stone-50 bg-opacity-75 p-2 backdrop-blur-md`}
    >
      {renderResults()}
    </ul>
  );
